# Contributing to Session Atlas

Community project of AI HUB Tilburg. PRs welcome from everyone.

## Ground rules

1. **Zero runtime dependencies.** `session_atlas.py` is Python 3 stdlib only; `index.html` is one self-contained file with no CDN/external requests. PRs that add a package manager will be declined — that constraint is the product.
...
```bash
python session_atlas.py --index-only   # rebuild data.json
python session_atlas.py --serve        # serve on 127.0.0.1:8877
```
